...
contract SupplyChain is Ownable, ConsumerRole, DistributorRole, FarmerRole, RetailerRole {
  ...
  struct Item {
    ...
    address ownerID;  // Metamask-Ethereum address of the current ownerAddress as the product moves through 8 stages
  ...
  // In the constructor set 'ownerAddress' to the address that instantiated the contract
  // and set 'sku' to 1
  // and set 'upc' to 1
  ...
  function kill() public {
    if (msg.sender == ownerAddress) {
      selfdestruct(address(uint160(ownerAddress)));
    }
  }
  ...
  function buyItem(uint _upc) public payable forSale(_upc) paidEnough(items[_upc].productPrice) checkValue(_upc) onlyDistributor
  {
    ...
    address payable _farmer = address(uint160(items[_upc].originFarmerID));
    _farmer.transfer(items[_upc].productPrice);
    // emit the appropriate event
    emit Sold(_upc);
  }
